...
const difficulties = {
  difficultiesInner: ["Choose", "Easy", "Medium", "Hard"],
};
...
export default function Exercises() {
  ...
  const [difficultiesSelection, setDifficulty] = useState(
    difficulties.difficultiesInner[0]
  );
  const [exerciseTypeSelection, setTopic] = useState(
    exercises.exercisesInner[0]
  );
  ...
  async function fetchExercises() {
    ...
    try {
      const taskResponse = await axios.get(
        backendApiLink + `Task/${difficultiesSelection}/${test}`
      );
      const tasks = taskResponse.data;

      const userResponse = await axios.get(
        backendApiLink + `Logged/user/${username}`
      );
  ...
  async function fetchExercisesTypes() {
    try {
      const result = await axios.get(backendApiLink + `TaskType`, {
